...
if %DEMO_ARG% == "demo" (
  echo Making PDSynth Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making PDSynth Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild PDSynth-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild PDSynth.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild PDSynth.sln /t:PDSynth-app;PDSynth-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\PDSynth.aaxplugin\Contents\Win32\PDSynth.aaxplugin --out .\build-win\aax\bin\PDSynth.aaxplugin\Contents\Win32\PDSynth.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\PDSynth.aaxplugin\Contents\x64\PDSynth.aaxplugin --out .\build-win\aax\bin\PDSynth.aaxplugin\Contents\x64\PDSynth.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\PDSynth.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\PDSynth.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "PDSynth Installer" ".\installer\PDSynth Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\PDSynth Installer.exe" ".\installer\PDSynth Demo Installer.exe"
  REM -del ".\installer\PDSynth Installer.exe"
  REM -)
