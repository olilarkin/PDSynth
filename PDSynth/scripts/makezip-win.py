...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\PDSynth Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\PDSynth Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\PDSynth manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\PDSynth.vst3\\Contents\\x86_64-win\\PDSynth.vst3",
      projectpath + "\\build-win\\PDSynth_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\PDSynth-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\PDSynth-app_x64.pdb"  
  ]
