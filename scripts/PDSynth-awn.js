/* Declares the PDSynth Audio Worklet Node */

class PDSynthController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "PDSynth", options);
  }

  static importScripts (actx) {
    var origin = "https://olilarkin.github.io/PDSynth/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/PDSynth-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/PDSynth-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
